...
export async function POST(req: NextRequest) {
  ...
  const {
    studentId, sessionDate, paragraphs,
  ...
  if (!paragraphs?.length) return Response.json({ error: 'At least one paragraph required' }, { status: 400 })
  ...
  const responses = [
    ...
    ...paragraphs.map((p: { text: string; misspokeCount: number }, i: number) => ({
      session_id: session.id,
      hunk_number: i + 1,
      question_type: 'OPEN',
      question_text: 'Paragraph',
      captured_answer: p.text,
      expected_answer: '',
      misspoke_count: p.misspokeCount,
    })),
  ]
