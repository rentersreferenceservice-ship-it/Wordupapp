...
export default function BackgroundText() {
  ...
  if (pathname.startsWith('/practice-tools')) return null
  if (pathname.startsWith('/practitioner-services')) return null
  ...
  if (pathname.startsWith('/testimonials')) return null
  if (pathname.startsWith('/reflections')) return null
  if (pathname.startsWith('/lessons')) return null
  if (pathname.startsWith('/type-to-talk')) return null
  const content = [
    ...
    { color: 'rgba(21,128,61,0.55)', text: 'What process do plants use to make their own food? What tool does a speller use to communicate? What does the topic sentence tell the reader at the start of a paragraph?' },
    ...
    { color: 'rgba(234,88,12,0.55)', text: 'Name two things a letterboard gives a person. Name two things the colonists wanted from the American Revolution. Name two facts the text tells us about the Artemis Mission.' },
