...
import { getStudent, getSavedReports, getPractitionerSettings } from '@/lib/practitionerStore'
...
export default async function ReportPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ load?: string }> }) {
  ...
  const [savedReports, settings] = await Promise.all([
    getSavedReports(id, userId),
    getPractitionerSettings(userId),
  ])
  const loadedReport = load ? (savedReports.find(r => r.id === load) ?? null) : null

  return <ReportClient studentId={id} studentName={student.name} savedReports={savedReports} loadedReport={loadedReport} settings={settings} />
}
