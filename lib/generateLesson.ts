...
- CRITICAL — THE MOST COMMON FAILURE: Do NOT load the question stem with facts from the hunk text and then ask for one of those same facts. This is the single most frequent error. Example of what NEVER to do: The hunk says "HARVARD UNIVERSITY is located in Cambridge, Massachusetts." The AI writes: "Harvard University is located in Cambridge, Massachusetts. What city is Harvard in?" → CAMBRIDGE. This question contains the answer twice — once in the stem, once as the answer. It adds nothing. Instead, take the anchor (Harvard) and ask for something the hunk never says: "Harvard University is part of the Ivy League. Name another Ivy League school." → YALE / PRINCETON / COLUMBIA. Or: "Harvard is the oldest university in the United States. It was founded in _____." → 1636 (if not already in the hunk). The student must supply knowledge the lesson did not give them — that is the entire point of PRIOR KNOWLEDGE.
